...
def draw_background_box(textID, update):
    ...
    if textID == "attemptChange":
        ...
        height = (dimensions.current_h / 100) * \
...
def format_image(logo, scaling_factor, x, y):
    ...
    logo_position.centerx = (dimensions.current_w / 100) * \
...
def init_arrow(image):
    ...
    scaling_factor = 12
    image_dimensions = raw_logo.get_rect().size
    x_scale = (((float(dimensions.current_h / 100) * scaling_factor))
               * float(image_dimensions[0])) / (float(image_dimensions[1]))
    scaled_logo_main = pygame.transform.scale(
        raw_logo, ((int(x_scale)), int((dimensions.current_h / 100) * scaling_factor)))  # scale the image to fit
    logo_position = scaled_logo_main.get_rect()
    return scaled_logo_main, logo_position
...
def draw_arrow(raw_logo, x, y, arrow):
    ...
    if arrow == "right":  # top side arrow
        ...
        surface.blit(bottom_arrow_surface, (
...
def place_text(text, Color, textFont, loc_x, loc_y, textID):
    ...
    rendered_message = textFont.render(
        str(text), True, color)  # used for the regular pygame font rendering
    # rendered_message = textFont.render(text, color)

    text_position = rendered_message.get_rect(
    )  # get the size of the rendered text (regular
    text_position.centerx = (dimensions.current_w / 100) * \
        loc_x  # specify where the center of the message should be on the x axis
    text_position.centery = (
        dimensions.current_h / 100) * loc_y  # specify where the cen
    ...
    if textID == "attempt1":
        ...
        draw_attempt_box(attempt_timer1.seconds, attempt1_image[
                         1])  # place the pretty frame over the box
        place_image(attempt1_image, False)
        surface.blit(rendered_message, text_position)
                     # place the text on the screen
        pygame.display.update(
            attempt1_image[1])  # just update the part of the screen where the text is.

    if textID == "attempt2":
        print("Drawing Attempt box for attempt2")
        draw_attempt_box(attempt_timer2.seconds, attempt2_image[
                         1])  # place the pretty frame over the box
        place_image(attempt2_image, False)
        surface.blit(rendered_message, text_position)
                     # place the text on the screen
        pygame.display.update(
            attempt2_image[1])  # just update the part of the screen where the text is.

    if textID == "attempt3":
        print("Drawing Attempt box for attempt3")
        draw_attempt_box(attempt_timer3.seconds, attempt3_image[
                         1])  # place the pretty frame over the box
        ...
        # screen where the text is.
        pygame.display.update(
            attempt3_image[1])  # just update the part of the screen where the text is.
...
def start_attempt_timer1():
    ...
    if attempt_timer1.seconds > 0:
        place_text(
            attempt_timer1.seconds, "black", attempt_timer_font, 76 +
                offset + e,
                   83 + yoffset, "attempt1")  # prints the main timer. #update the timerplace_text
...
def start_attempt_timer2():
    ...
    if attempt_timer2.seconds > 0:
        place_text(
            attempt_timer2.seconds, "black", attempt_timer_font, 83 +
                offset + e,
                   83 + yoffset, "attempt2")  # prints the main timer. #update the timerplace_text
...
def start_attempt_timer3():
    ...
    if attempt_timer3.seconds > 0:
        place_text(
            attempt_timer3.seconds, "black", attempt_timer_font, 90 +
                offset + e,
                   83 + yoffset, "attempt3")  # prints the main timer. #update the timerplace_text
        ...
        if attempt_timer2.seconds == 0:
            ...
            if attempt_timer3.seconds == 0:
                ...
                attempt_timer3.transfer(
                    attempt_timer2.seconds)  # transfer second clock to 3rd
                attempt_timer2.transfer(
                    attempt_timer1.seconds)  # transfer first clock to 2nd
...
def break_mode():
    ...
    place_text(main_timer.break_status(), "white",
               main_timer_break_font, 50, 40, "breakTimer")
    ...
    place_text(main_timer.openerStatus(), "red",
               opener_change_font, 50, 73, "openerChange")
    #draw_background_box("openerChange", True)
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_KP_ENTER:  # EXIT CONDITION
                    ...
                    if len(new_time) == 5: #this means hours were entered.
                        ...
                        if len(new_time) == 0:  # this means the user entered nothing, which we default to normal operation.
                            ...
                            if seconds > 59:  # handle the weird input of 90 seconds = 1:30 we need to add minutes. 60-99 needs to add a minute
                                ...
                                seconds = (seconds - 60)
                            if minutes > 59:
                                main_timer.hours = main_timer.hours + 1 #add another hour.
                                minutes = (minutes - 60)
                            else:
                                main_timer.hours = 0
                            ...
                            if total_time <= 240 and breakMode:  # if the requested time is less than 4 minutes, then do normal operation.
                                ...
                                place_text(
                                    "0:00:00", "gray", main_timer_font, 50, 70, "timerChange")  # "hides" the main timer..
                                place_text(
                                    main_timer.status(), "white", main_timer_font, 50, 70, "timerChange")  # prints the main timer.
                                # update the class value
                    ...
                    if len(new_time) > 1:  # this means the string is not empty.
                        ...
                        new_time = new_time[
                            :-1]  # backspace will remove the last input
                        # update the time on the screen
                        place_text(
                            stringToclock(new_time), color, main_timer_font, 50, 70, "timerChange")
                    elif len(new_time) == 1:
                        print(
                            "string has 1 character left, emptying the string")
                        new_time = ''
                        place_text(
                            stringToclock(new_time), color, main_timer_font, 50, 70, "timerChange")
...
def attempt_timer_reset():
    ...
    place_text(attempt_timer1.seconds, "gray", attempt_timer_font, 76 +
               offset + e, 83 + yoffset, "attempt1")  # print the new values on the screen
    place_text(attempt_timer2.seconds, "gray", attempt_timer_font, 83 +
               offset + e, 83 + yoffset, "attempt2")  # print the new values on the screen.
    place_text(attempt_timer3.seconds, "gray", attempt_timer_font, 90 +
               offset + e, 83 + yoffset, "attempt3")  # print the new values on the screen
...
def system_reset():
    ...
    place_text(main_timer.status(), "white",
               main_timer_font, 50, 70, "mainTimer")
...
def attempt_change():
    ...
    if not breakMode and not revealDecision and main_timer.currently_reset:
        ...
        rightxLocation = 0
        rightyLocation = dimensions.current_h / \
            100 * 10  # location of bottom arrow.

        leftxLocation = dimensions.current_w
        leftyLocation = dimensions.current_h / \
            100 * 23  # location of bottom arrow.
        ...
        while stayLooped:
            ...
            draw_arrow(right_arrow_image, rightxLocation,
                       rightyLocation, "right")  # draw the arrow.
            ...
            draw_arrow(left_arrow_image, leftxLocation,
                       leftyLocation, "left")  # draw the arrow.
    ...
    print("BreakMode: " + str(breakMode) + " " + "revealDecision: " +
          str(revealDecision) + " " + "maintimer_reset " + str(main_timer.currently_reset))
...
def clock_event():
    ...
    if main_timer.running or main_timer.empty:
        ...
        if breakMode and not attempt_change_mode:
            ...
            place_text(main_timer.break_status(), "white", main_timer_break_font, 50, 40, "breakTimer")
...
def get_caps_lock():
    out = subprocess.run(['xset','-q'],stdout=subprocess.PIPE) # query the OS for the state of the keys
    out_text = out.stdout.decode('utf-8') #get the result and convert to a string
    match = re.search('Caps Lock:\s*(off|on)', out_text) #use a regex to search for the status (on/off)
    if match.group(1) == "on":
        capsLockOn = True
    else:
        capsLockOn = False
        ...
        for event in GAME_EVENTS.get():
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_v or event.key == pygame.K_m: #IPF White button pushed, which means select the desired remote. 
                    ...
                    place_text("Press the white button to confirm selection as:", "white",menu_font, 50,20,"mainTimer")
                    place_text(selected + " remote", "red",menu_font, 50,30,"mainTimer")
                ...
                if event.key == pygame.K_b: #red button.
                    ...
                    if selected == "left":
                        selected = "chief"
                        if IPF: #if the set is configured as an IPF set, draw the appropriate remote images
                            place_image(left_remote_ipf_image, True) # make the left remote white
                            place_image(chief_remote_ipf_selected_image, True) # select the chief remote
                        else:
                        ...
                        selected = "right"
                        if IPF:
                            place_image(chief_remote_ipf_image, True) # make the chief remote white
                            place_image(right_remote_ipf_selected_image, True) #select the right remote
                        else:
                        ...
                        selected = "left"
                        if IPF:
                            place_image(right_remote_ipf_image, True) # select the right remote
                            place_image(left_remote_ipf_selected_image, True) # select the left remote
                        else:
...
attempt_timer1_event = pygame.USEREVENT + \
    2  # this happnes every second to decimate the attempt 1 timer
attempt_timer2_event = pygame.USEREVENT + \
    3  # this happnes every second to decimate the attempt 1 timer
attempt_timer3_event = pygame.USEREVENT + \
    4  # this happnes every second to decimate the attempt 1 timer
reveal_lights_event = pygame.USEREVENT + \
    5  # this event happens 2 seconds after the lights are shown.
clear_lights_event = pygame.USEREVENT + \
    6  # this event happens 10 seconds after the lights are shown
watchdog_event = pygame.USEREVENT + \
    7  # this will be the timer thats checks for inadvertant button pushes.
...
place_text(main_timer.status(), "white", main_timer_font,
           50, 70, "mainTimer")  # prints the main timer.
    ...
    for event in GAME_EVENTS.get():
        ...
        if event.type == sync_check_event and not revealDecision:
            ...
            if not leftSync:  # if leftSync = True, don't draw the caution triangle. when a ping event comes in leftSync = True
                ...
                print(
                    "Left currently desync status: " + str(left_currently_desync))
            ...
            if not chiefSync:  # and not chief_currently_desync:
                ...
                print(
                    "Chief currently desync status: " + str(chief_currently_desync))
            ...
            if not rightSync:  # and not right_currently_desync:
                ...
                print(
                    "right currently desync status: " + str(right_currently_desync))
                if not breakMode and not right_currently_desync:  # if not break mode, then go ahead and draw.
                    ...
                    draw_background_box(
                        "right", True)  # this should handle the case where a remote de syncs mid decision.
                    reset_rightLights()
        ...
        if event.type == attempt_timer1_event:
            ...
            attempt_timer1.tick()
            place_text(attempt_timer1.seconds, "black", attempt_timer_font, 76 +
                       offset + e, 83 + yoffset, "attempt1")  # print the new values on the screen.
            if attempt_timer1.empty:
                # if the clock is empty, then clear the queue from future
                # repeat calls.
                pygame.time.set_timer(
                    attempt_timer1_event, 0)  # Halt the repeating events.
                pygame.event.get(attempt_timer1_event)
        ...
        if event.type == attempt_timer2_event:
            ...
            place_text(attempt_timer2.seconds, "black", attempt_timer_font, 83 +
                       offset + e, 83 + yoffset, "attempt2")  # print the new values on the screen.
            if attempt_timer2.empty:
                ...
                # repeat calls.
                pygame.time.set_timer(
                    attempt_timer2_event, 0)  # Halt the repeating events.
                pygame.event.get(attempt_timer2_event)
                                 # this should remove any late main timer
                                 # events in the queue.
                place_text(attempt_timer2.seconds, "gray", attempt_timer_font, 83 +
        ...
        if event.type == attempt_timer3_event:
            ...
            place_text(attempt_timer3.seconds, "black", attempt_timer_font, 90 +
                       offset + e, 83 + yoffset, "attempt3")  # print the new values on the screen.
            if attempt_timer3.empty:
                ...
                # repeat calls.
                pygame.time.set_timer(
                    attempt_timer3_event, 0)  # Halt the repeating events.
                pygame.event.get(attempt_timer3_event)
                                 # this should remove any late main timer
                                 # events in the queue.
                place_text(attempt_timer3.seconds, "gray", attempt_timer_font, 90 +
        ...
        if event.type == main_timer_event:
            ...
            if not breakMode and not attempt_change_mode:  # regular operation
                ...
                if main_timer.minutes == 0 and main_timer.seconds == 0 and main_timer.hours == 0:
                    ...
                    if main_timer.minutes > openerChange - 1 or main_timer.hours > 0:
                        ...
                        if main_timer.minutes == openerChange - 1 and main_timer.seconds == 59:
                            ...
                            # time.
                            place_text(
                                "0:00:00", "gray", opener_change_font, 50, 73, "openerChange")
                                       # this is used strictly to clear the
                                       # space needed for the next message.
                            place_text(
                                "NO MORE CHANGES", "red", no_more_changes_font, 50, 73, "openerChange")
        ...
        if event.type == watchdog_event:
            ...
            pygame.time.set_timer(
                watchdog_event, 0)  # Halt the repeating events.
        ...
        if event.type == reveal_lights_event:
            ...
            if lc.configured: #if liftingcast configuration is setup
                try:
                    t1 = Thread(target=lc.drl_decisions_to_liftingcast_decisions, args=[leftGoodlift, leftRed, leftBlue,leftYellow,chiefGoodlift,chiefRed,chiefBlue,chiefYellow,rightGoodlift,rightRed, rightBlue,rightYellow])
                    t1.daemon = True
        ...
        if event.type == clear_lights_event:
            ...
            print("Clear lights event, attempt_change_mode status: " +
